...
def map_dict_keys(obj: Dict, map_rules: Dict[str, str], only_given_fields: bool = False):
...
def split_query_to_term_args(query: str) -> Dict[str, Any]:
    def get_value(term: str):
        return term[term.index(':') + 1:].strip()

    def get_key(term: str):
...
def get_api_id(args: Dict[str, Any]):
...
def crowdstrike_submit_url_command(client: Client, args: Dict[str, Any]):
...
def crowdstrike_analysis_overview_command(client: Client, args: Dict[str, Any]):
...
def crowdstrike_search_command(client: Client, args: Dict[str, Any]):
...
def crowdstrike_analysis_overview_refresh_command(client: Client, args: Dict[str, Any]):
